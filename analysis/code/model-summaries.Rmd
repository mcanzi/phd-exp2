...
F.gamms <- ERP %>% 
  ...
  filter(electrode %in% c("F1", "F3", "F5", "FC1", "FC3", "FC5", 
                          "F2", "F4", "F6", "FC2", "FC4", "FC6")) %>% 
  filter(condition == "control" | condition == "target") %>% 
  mutate(emisphere = if_else(electrode %in% c("F1", "F3", "F5", "FC1", "FC3", "FC5"), "left", "right"), 
         condition = as.ordered(condition)) %>% 
  mutate(emi.cond = interaction(emisphere, condition)) %>% 
  mutate(emi.cond = as.ordered(emi.cond))
contrasts(ERP.gamms$condition) <- "contr.treatment"
contrasts(ERP.gamms$emi.cond) <- "contr.treatment"
```
...
```{r}
plot_smooths(model = first.model, 
             series = time, 
             comparison = condition)
             #facet_terms = emisphere,
             #split = list(emi.cond = c("emisphere", "condition")))
```
...
P.gamms <- ERP %>% 
  select(subject, amplitude, condition, time, electrode) %>% 
  filter(electrode %in% c("P1", "P3", "P5", "CP1", "CP3", "CP5", 
                          "P2", "P4", "P6", "CP2", "CP4", "CP6")) %>% 
  filter(condition == "control" | condition == "target") %>% 
  mutate(emisphere = if_else(electrode %in% c("P1", "P3", "P5", "CP1", "CP3", "CP5"), "left", "right"), 
         condition = as.ordered(condition)) %>% 
  mutate(emi.cond = interaction(emisphere, condition)) %>% 
  mutate(emi.cond = as.ordered(emi.cond))
contrasts(ERP.gamms$condition) <- "contr.treatment"
...
```{r}
second.model <- bam(amplitude ~ 
                     s(time, k = 40) + 
                     condition + 
                     s(time, by = emi.cond, k = 50) +
                     s(time, subject, bs = "fs", m = 1) + 
                     s(time, electrode, bs = "fs", m = 1), 
                   method = "ML", data = ERP.gamms)
```

```{r}
plot_smooths(model = second.model, 
             series = time, 
             comparison = condition,
             facet_terms = emisphere,
             split = list(emi.cond = c("emisphere", "condition")))
