...
summarised.model <- function(TP1, TP2, type = "lmer") {
  ...
  lmer.data <- left_join(ERP %>%
    ...
    summarise(avg.mean = mean(amplitude.mean),) %>% ungroup,
...
emmeans(summarised.model(75, 125, "lm"), pairwise ~ condition | region)
```
...
```{r, message = F}
anova(summarised.model(280, 320, type = "lm"))
emmeans(summarised.model(280, 320, "lm"), pairwise ~ condition | region)
```

```{r, message = F}
anova(summarised.model(575, 625, type = "lm"))
emmeans(summarised.model(574, 625, "lm"), pairwise ~ condition | region)
```
