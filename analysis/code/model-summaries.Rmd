...
ERP <- bind_rows(read.csv("../data/EXP2.csv"), read.csv("../data/EXP1_pt1.csv"),
...
```{r, message = F}
anova(summarised.model(75, 125, type = "lmer"))
emmeans(summarised.model(75, 125, "lmer"), pairwise ~ condition | region)
```

```{r, message = F}
anova(summarised.model(150, 200, type = "lmer"))
```

```{r, message = F}
anova(summarised.model(290, 310, type = "lmer"))
```

```{r, message = F}
anova(summarised.model(575, 625, type = "lmer"))
emmeans(summarised.model(75, 125, "lmer"), pairwise ~ condition | region)
```
