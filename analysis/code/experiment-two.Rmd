...
int_scalp_plot <- function(TP1, TP2, cond = "control") {
  ...
  ScalpPlotT <- ggplot(interpTopo, aes(x = x, y = y, fill = Amplitude)) +
    ...
    theme_topo() +
    scale_fill_viridis(option = "turbo", limits = c(-3, 3), guide = "colorbar", oob = squish, 
...
int_compare_scalps <- function(tp1, tp2) {
  ...
  if (tp1 <= 9 && tp2 <= 9) {
    ...
    common.legend = TRUE), 
    top = text_grob(paste0("Mean amplitude (00", tp1, "-00", tp2, " ms) cubic-spline interpolation scalp map"),
                    face = "bold")) }
  else if (tp1 <= 9 && tp2 > 9) {
    final_plot <- annotate_figure(ggarrange(
    target_map + ggtitle("Mismatch"),
    control_map + ggtitle("Match"),
    common.legend = TRUE), 
    top = text_grob(paste0("Mean amplitude (00", tp1, "-0", tp2, " ms) cubic-spline interpolation scalp map"),
                    face = "bold")) }
  else if (tp1 > 9 & tp1 <= 99 && tp2 > 9 && tp2 <= 99) {
    final_plot <- annotate_figure(ggarrange(
    target_map + ggtitle("Mismatch"),
    control_map + ggtitle("Match"),
    common.legend = TRUE), 
    top = text_grob(paste0("Mean amplitude (0", tp1, "-0", tp2, " ms) cubic-spline interpolation scalp map"),
                    face = "bold")) }
    else if (tp1 > 9 & tp1 <= 99 && tp2 > 99) {
    final_plot <- annotate_figure(ggarrange(
    target_map + ggtitle("Mismatch"),
    control_map + ggtitle("Match"),
    common.legend = TRUE), 
    top = text_grob(paste0("Mean amplitude (0", tp1, "-", tp2, " ms) cubic-spline interpolation scalp map"),
                    face = "bold")) }
  else if (tp1 > 99 && tp2 > 99) {
    final_plot <- annotate_figure(ggarrange(
    target_map + ggtitle("Mismatch"),
    control_map + ggtitle("Match"),
    common.legend = TRUE), 
    top = text_grob(paste0("Mean amplitude (", tp1, "-", tp2, " ms) cubic-spline interpolation scalp map"),
                    face = "bold")) }
...
for (i in 1:140) {
  z <- int_compare_scalps(T1, T2)
  ggsave(paste0("gif/frame", i+100, ".png"), z, dpi = "print")
  T1 <- T1 + 5
  T2 <- T2 + 5
}
...
gifski(png_files, gif_file = "turbo.gif", width = 800, height = 500, delay = 0.04)
```
...
for (i in 1:140) {
  z <- int_compare_scalps(T1, T2)
  ggsave(paste0("gif2/frame", i+100, ".png"), z, dpi = "print")
  T1 <- T1 + 5
  T2 <- T2 + 5
}

library(gifski)
png_files <- list.files("gif2/", pattern = ".*png$", full.names = TRUE)
gifski(png_files, gif_file = "rocket.gif", width = 800, height = 500, delay = 0.04)
```
